...
public abstract class ArbolBinario<T> implements Coleccion<T> {
    ...
    protected class Vertice implements VerticeArbolBinario<T> {
        ...
        private boolean equalsAux(Vertice v1, Vertice v2){
			...
			if((!v1.elemento.equals(v2.elemento)))
				return false;
			return equalsAux(v1.izquierdo, v2.izquierdo) 
                && equalsAux(v1.derecho, v2.derecho);
		}
    }
    ...
    public int profundidad() {
        if(elementos == 0)
			return 0;
		return treeDepth(raiz);
    }
    
    private  int treeDepth(Vertice root) {
		if (root == null)
			return -1;
        int lh = 1+treeDepth(root.izquierdo);
        int rh = 1+treeDepth(root.derecho);
		if(lh > rh)
			return lh; 
	 return rh;
    }
    ...
    @Override public String toString() {
        ...
        for (int i = 0; i < p; i++)     /* Al inicio, no dibujamos ninguna rama. */
            rama[i] = false;
