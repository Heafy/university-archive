...
dilatacion y contraccion a traves de transformaciones lineales
'''
import pylab as pl

pl.figure("Operadores de Dilatacion y Contraccion")
...
if(factor < 1):
	strFactor = "Factor de contraccion"
elif(factor > 1):
	strFactor = "Factor de dilatacion"
...
factorV = ax.arrow(x0, y0, (x1-x0)*factor, (y1-y0)*factor, head_width=0.3, head_length=0.3, color='green')
# Creamos las labels para mostrar su informacion
pl.legend([original, factorV], ['Original', strFactor])
ax.set_xlabel('Eje x')
ax.set_ylabel('Eje y')
