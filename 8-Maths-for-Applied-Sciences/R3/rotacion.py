...
fig = pl.figure("Operadores de rotacion")
...
b = Vector3D([x0,x1],[y0,costy-(sintz)],[z0,sinty+costz], mutation_scale=20, lw=1, arrowstyle="-|>", color="k")
ax.add_artist(b)
# Rotacion alrededor del eje y 
c = Vector3D([x0,costx+sintx],[y0,y1],[z0,sintx+costz], mutation_scale=20, lw=1, arrowstyle="-|>", color="k")
ax.add_artist(c)
# Rotacion alrededor del eje z
d = Vector3D([x0, costx-sinty],[y0,sintx+costy],[z0,z1], mutation_scale=20, lw=1, arrowstyle="-|>", color="k")
ax.add_artist(d)


# Tomamos el arreglo de las coordenadas creadas anteriormente y lo casteamos a
